...
namespace VG.Internal
{
    public abstract class Manager : Initializable
    {
        ...
        public sealed override void Initialize()
        {
            ...
            foreach (var service in _services) _waitInitializations.Add(service);
        ...
        private IEnumerator WaitInitializations()
        {
            ...
            while(!initialized)
            {
                ...
                { 
                    bool isNotSupportedService = waitableInitialization is Service 
                        && ((Service)waitableInitialization).supported == false;

                    if (!waitableInitialization.initialized && !isNotSupportedService)
                    {
                        initialized = false;
                        break;
                    }
